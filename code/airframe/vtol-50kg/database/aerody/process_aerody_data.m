...
config = 'landgear';
% 2.1力
temp  = sortrows(force, [2 1]); % 按照beta,alpha排序
alpha = unique(temp.alpha);
beta  = unique(temp.beta);
field = {'cx', 'cy', 'cz'};
for j = 1 : 3
...
% 2.2力矩
temp  = sortrows(moment, [2, 1]); % 按照beta,alpha排序
alpha = unique(temp.alpha);
beta  = unique(temp.beta);
field = {'cll', 'cm', 'cn'};
for j = 1 : 3
...
if ~ismember(0, aerody.left_aileron.cx.beta)
    % 获取右副翼beta=0的数据
    ra_beta0_idx = find(aerody.right_aileron.cx.beta == 0);
...
if ~ismember(0, aerody.left_rudder.cx.beta)
    % 获取右方向舵beta=0的数据
    rr_beta0_idx = find(aerody.right_rudder.cx.beta == 0);
...
for cidx = 1:length(configs)
    ...
    for fidx = 1:length(fields)
        ...
        % 获取当前系数结构
        current_coeff = aerody.(config).(field).coeff;
        current_beta = aerody.(config).(field).beta;

        % 检查并添加每个负beta
        for beta = aerody.(config).(field).beta(:)'
            % 如果负beta不存在且正beta存在
            if ~ismember(-beta, current_beta) && ismember(beta, current_beta)
                % 根据系数类型确定数据生成规则
                if ismember(field, {'cx', 'cz', 'cm'})
                    % 对称系数：直接复制
                    pos_idx  = find(current_beta == beta); % 获取正beta的索引
                    new_data = current_coeff(:, pos_idx, :);
                else % cll, cy, cn
                    if startsWith(config, 'left')
                        config_right = strrep(config, 'left', 'right');
                        pos_idx  = find(aerody.(config_right).(field).beta == beta);
                        new_data = -aerody.(config_right).(field).coeff(:, pos_idx, :);
                    elseif startsWith(config, 'right')
                        config_left = strrep(config, 'right', 'left');
                        pos_idx  = find(aerody.(config_left).(field).beta == beta);
                        new_data = -aerody.(config_left).(field).coeff(:, pos_idx, :);
                    else
                        pos_idx  = find(current_beta == beta); % 获取正beta的索引
                        new_data = -current_coeff(:, pos_idx, :);
                    end
                ...
                % 根据维度处理数据拼接
                if ismatrix(current_coeff)  % 基础构型和起落架
                    current_coeff = [current_coeff, new_data];
                else  % 舵面构型
                    current_coeff = cat(2, current_coeff, new_data);
                end
            end
        end
        ...
        if ismatrix(current_coeff)
            sorted_coeff = current_coeff(:, sort_idx);
        else
            sorted_coeff = current_coeff(:, sort_idx, :);
        end
...
for cidx = 1:length(configs)
    ...
    if ismember(config, {'left_aileron', 'right_aileron', 'left_rudder', 'right_rudder'})
        ...
        for j = 1:length(fields)
            ...
            aerody.(config).(field).coeff = current_coeff(:, :, sort_idx);
        end
        ...
        if ~ismember(0, sorted_delta)
            ...
            for j = 1:length(fields)
                field = fields{j};
                current_coeff = aerody.(config).(field).coeff;
                zero_slice = zeros(size(current_coeff, 1), size(current_coeff, 2));
                new_coeff = cat(3, current_coeff, zero_slice);
...
configs = {'left_aileron', 'right_aileron', 'left_rudder', 'right_rudder'};
for cidx = 1 : length(configs)
    config = configs{cidx};
