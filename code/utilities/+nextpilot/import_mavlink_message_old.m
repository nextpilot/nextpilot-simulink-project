...
if isempty(exts) && strcmpi(name, 'base')
    save_mavlink_to_ws(save_target, mavlink)
elseif strcmpi(exts, '.sldd')
    save_mavlink_to_sldd(save_target, mavlink)
end

function  save_param_to_ws(ws, mavlink)


function save_mavlink_to_sldd(file, mavlink)
if exist(file,'file')
